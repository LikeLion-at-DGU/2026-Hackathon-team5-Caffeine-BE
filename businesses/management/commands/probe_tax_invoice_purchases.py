"""전자세금계산서 매입 Real CODEF 호출 검증용 관리 명령.

실제 사업자 + 카카오 간편인증을 사용하여
전자세금계산서 통합 API의 매입 데이터를 조회한다.
...
# payload 필드명 / 환경변수 / 입력 안내 / getpass 여부
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        provided = [
            name
            for name, value in two_way_values.items()
            if value not in (None, "")
        ]

        if (
            provided
            and len(provided) != len(TWO_WAY_ARG_NAMES)
        ):

            missing_two_way = [
                name
                for name in TWO_WAY_ARG_NAMES
                if two_way_values[name] in (None, "")
            ]
        ...
        try:
            ...
            normalized_items = normalize_tax_invoices(
                raw,
