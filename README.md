...
2. 카페 특화 **면세 원재료 의제매입세액 추정 및 검토 지원**으로 놓치기 쉬운 공제 후보를 보여주며,
...
- **1차 (Deterministic Engine)**: 금융 거래 정규화, 의제매입세액 추정, 원천징수/4대보험 계산 등 규칙 기반 처리가 가능한 영역을 Rule-based 엔진으로 처리
...
- **우유/생과일 등 면세 식자재 후보 식별** 및 2026년 개인 음식점업 특례율을 가정한 의제매입세액 추정
...
- 시급 × 월 근무시간 기반 급여·원천징수·4대보험 계산 및 주민등록번호 AES-256 암호화 저장
- 주휴수당은 주별 소정근로시간·소정근로일·개근 정보를 추가한 뒤 지원 예정
