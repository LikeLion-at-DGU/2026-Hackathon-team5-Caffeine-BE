    ...
    def get(self, request, business_id):
        ...
        try:
            ...
            file_field = report_service.get_report_file(
                business_id, year_month, params["format"]
            )
        except ReportServiceError as exc:
            return _error_response(exc.code, exc.message, exc.status_code)

        content_type = "text/csv; charset=utf-8" if params["format"] == "csv" else "application/pdf"
        response = HttpResponse(file_field.read(), content_type=content_type)
        response["Content-Disposition"] = (
            f'attachment; filename="cafe_assistant_{year_month}.{params["format"]}"'
        )
        return response
